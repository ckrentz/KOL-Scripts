...
// Bug: flush_wanderers no longer works, since wanderers no longer appear at shrines
//
...
// switch pants.
//
// pillkeeper semi at beginning of run if collecting semirares and no
// Fortune Cookie counter
//
// KGB Briefcase buffs
// Bird-A-Day Calendar?
// Chocolates?
//
// Consider whether count script could take over all semirare handling
// rather than only that while we call external scripts
//
...
// Bug: Pantsgiving can give you fullness, which we will use for adventures.
//      However, if it gives you more fullness during such, we don't use it.
//
// Voting Booth:
...
familiar item_familiar = define_property( "VMF.ItemFamiliar", "familiar", "Pocket Professor" ).to_familiar();
...
string blessing = define_property( "VMF.FriarsBlessing", "string", "familiar" );

// What to drink for a nightcap. If "" or "none", will not overdrink.

item nightcap = define_property( "VMF.Nightcap", "item", "none" ).to_item();
...
location_list semirare_locations = define_property( "VMF.SemirareLocations", "location", "", "list" );
...
int semirare_counter()
{
    string[] counters = get_property( "relayCounters" ).split_string( ":" );
    for ( int index = 0; index + 2 <= count( counters); index += 3 ) {
	string name = counters[ index + 1 ];
	if ( name == "Fortune Cookie" ) {
	    int counter = counters[ index ].to_int();
	    return ( counter < my_turncount() ) ? -1 : counter;
	}
    }
    // No counter is currently set
    return -1;
}

boolean will_hunt_semirares = false;	// Set to true if should_hunt_semirares and validation succeeds
int next_semirare = semirare_counter();	// Turn number of next semirare
...
static item CLAN_SPEAKEASY = $item[ Clan speakeasy ];
...
static item LUCKY_LINDY = $item[ Lucky Lindy ];
...
static item TEN_LEAF_CLOVER = $item[ ten-leaf clover ];
...
boolean have_speakeasy = clan_lounge contains CLAN_SPEAKEASY;
...
void validate_configuration()
{
    ...
    if ( should_hunt_semirares ) {
	// We use Luck Lindy to learn semirare counter.
	boolean inebriety_available = ( my_inebriety() < inebriety_limit() );
	boolean speakeasy_drinks_available = ( get_property( "_speakeasyDrinksDrunk" ).to_int() < 3 );

	will_hunt_semirares = speakeasy_drinks_available && inebriety_available;

	// Even if we won't actually hunt, validate the locations
	// All locations must be known to have a semirare adventure
	foreach index, loc in semirare_locations {
	    if ( !( known_semirare_locations contains loc ) ) {
		print( "VMF.SemirareLocations: '" + loc + "' is not known to have a semirare adventure - ignoring.", "red" );
		remove semirare_locations[ index ];
		// This could be an error, but just issue a warnig
	    }
	}

	// Must have at least two desired semirare_locations
	int locs = count( semirare_locations );
	if ( locs < 2 ) {
	    print( "VMF.SemirareLocations: must have at least two locations but has " + locs, "red" );
	    valid = false;
	}

	// Don't validate access to speakeasy if we won't actually use it.
	if ( speakeasy_drinks_available ) {
	    // Must have access to Clan Speakeasy with Lucky Lindy available
	    if ( !have_speakeasy ) {
		print( "VMF.HuntSemirares: must have at access to clan speakeasy to buy Lucky Lindys.", "red" );
		valid = false;
	    }

	    // There will be no priice if the speakeasy is closed or Lucky Lindy is not unlocked
	    if ( npc_price( LUCKY_LINDY ) == 0 ) {
		print( "VMF.HuntSemirares: your clan speakeasy does not dispense Lucky Lindys.", "red" );
		valid = false;
	    }
	}
    }
	...
	if ( spacegate_command != "" && !svn_exists( "veracity0-spacegate" )) {
...
static effect ON_THE_TRAIL = $effect[ On the Trail ];
...
static item PIZZA = $item[ slice of pizza ];
...
void free_song_slot( effect eff )
{
	...
	// Only Accordion Thief buffs can be shrugged - or need to be
	skill sk = eff.to_skill();
	if ( sk.class == ACCORDION_THIEF && sk.buff ) {
	    songs++;
...
string timespinner_replicator_option( int minutes_left )
{
    ...
    // *** We could declare it as a dependency
...
void protect_clovers()
{
    // If user doesn't care about clover protection, neither do we.
    if ( !get_property( "cloverProtectActive" ).to_boolean() ) {
	return;
    }

    // If we have no 10-leaf clovers in inventory, no problem
    int clovers = item_amount( TEN_LEAF_CLOVER );
    if ( clovers == 0 ) {
	return;
    }

    // Otherwise, diasassemble them
    use( clovers, TEN_LEAF_CLOVER );
}
...
boolean pocket_crumbs_available = false;
...
void combat_filter_setup( location loc )
{
    ...
    pocket_crumbs_available = have_equipped( PANTSGIVING );
...
{ 
    ...
    boolean should_olfact = can_olfact && olfaction_targets contains mon && have_effect( ON_THE_TRAIL ) == 0;
    ...
    if ( round == 0 ) {
	gave_meat = false;
	pocket_crumbs = !pocket_crumbs_available;
...
int turns_to_semirare()
{
    return next_semirare < 0 ? MAX_INT : ( next_semirare - my_turncount() );
}
...
boolean semirare_now()
{
    return will_hunt_semirares && ( next_semirare == my_turncount() );
}

boolean drink_lucky_lindy()
{
    int current_drunk = my_inebriety();
    int max_drunk = inebriety_limit();

    // We should have left enough inebriety left for this, but just in case
    if ( current_drunk >= max_drunk ) {
	return false;
    }

    // If we are not hunting semirares, nope
    if ( !will_hunt_semirares ) {
	return false;
    }

    // If we have consumed 3 speakeasy drinks today, nope
    if ( get_property( "_speakeasyDrinksDrunk" ).to_int() >= 3 ) {
	return false;
    }

    // If there is an active fortune cookie counter, not necessary
    if ( next_semirare >= 0 ) {
	return false;
    }
    ...
    // Drink a Lucky Lindy
    ode_up( 1 );
    cli_execute( "drink 1 " + LUCKY_LINDY );

    // Recalculate semirare counter
    next_semirare = semirare_counter();

    return next_semirare >= 0;
}

int collect_semirare()
{
    location choose_semirare_location()
    {
	// Must have at least two locations to cycle through
	if ( count( semirare_locations ) < 2 ) {
	    return NO_LOCATION;
	}

	location last = get_property( "semirareLocation" ).to_location();

	// Cycle through locations; take next location after most recent semirare
	...
	// If the most recent semirare was the last location, choose the first in the list
	return semirare_locations[ 0 ];
    }
    ...
    location loc = choose_semirare_location();
    if ( loc == NO_LOCATION ) {
	return 0;
    }

    string filter = choose_combat_filter( loc );
    boolean ignored = adv1( loc, 1, filter );

    // *** what is wrong here?

    // If found a semirare, no longer know when the semirare will come
    next_semirare = semirare_counter();

    // Drink another Lucky Lindy to find next semirare number.  If we have
    // already consumed 3 speakeasy drinks today or have no inebriety left,
    // this can fail. If so, semirare_counter will be MAX_INT.
    drink_lucky_lindy();

    // If we don't know when the next semirare will arrive or it will
    // not arrive in today's remaining turns, no need to leave inebriety
    // for Lucky Lindy.
    if ( turns_to_semirare() >= my_adventures() ) {
	int drunk_remaining = inebriety_limit() - my_inebriety();
	if ( drunk_remaining > 0  ) {
	    ode_up( drunk_remaining );
...
int fight_voter()
{
    ...
    string filter = choose_combat_filter( SHRINE );
    boolean ignored = adv1( SHRINE, 1, filter );
...
void do_adventure( int turns, location loc )
{
    ...
    // - Semirares - does consume a turn - but not in adventuring location
    // - Fightig a Vote Monster
    ...
    while ( turns > 0 ) {
	int semirare_delay = will_hunt_semirares ? turns_to_semirare() : MAX_INT;
	...
	// there is Barf Mountain quest to Lubricate the Tracks, we've
	// suited up, and then called this function - but no adventuring
	// actually happens.

	if ( delay == 0 ) {
	    print( "*** semirare_delay = " + semirare_delay );
...
void do_adventure_until_choice( int turns, location loc, int goal )
{
    ...
    // If we ran over our semirare counter outside of this method, need
    // to recalculate it. If there was no counterscript running to
    // collect it, we lost it.
    if ( will_hunt_semirares && next_semirare < my_turncount() ) {
	if ( next_semirare != -1 ) {
	    print( "*** Failed to collect a semirare = " + ( my_turncount() - next_semirare) + " turns ago." );
	}
	// Recalculate semirare counter
	next_semirare = semirare_counter();
	// Drink a Lucky Lindy if necessary
	drink_lucky_lindy();
    }
	...
	if ( adventure_until_choice( delay ) ) {
	    return;
	}
	turns -= delay;
    }
...
void do_adv1( int turnsUsed, location loc )
{
    ...
    // If we are hunting semirares and it is time for one, collect it
    if ( semirare_now() ) {
	collect_semirare();
    }
...
void use_gingerbread_city()
{
    ...
    if ( !gingerbread_city_available ||
	 !svn_exists("veracity0-gingerbread")) {
	return;
    }
...
void acquire_gear()
{
    ...
    // configured for it
    //
    // Since you must adventure to get these costumes, don't do it if we
    // can't adventure.
...
void flush_wanderers()
{
    // Flush out wanderers and a waiting time prank by visiting adventure.php 
    while ( my_adventures() > 0 ) {
	print( "Flushing out wandering monsters" );
	suit_up( SHRINE, hobo_or_default_familiar );
	string page = visit_url( SHRINE.to_url() );
	if ( page.contains_text( "fight.php" ) ) {
	    string filter = choose_combat_filter( SHRINE );
	    run_combat( filter );
	} else {
...
void dmt_fights()
{
    ...
    // Don't waste DMT fights on wandering monsters
    flush_wanderers();
...
void harvest_garden()
{
    if ( !svn_exists( "veracity0-garden" ) ) {
	return;
    }
...
void celebrate_arrrbor_day()
{
    ...
    if ( !svn_exists( "veracity0-arrrbor-day" ) ) {
	return;
    }

    // This will consume either 3 or 102 turns.
    // Collect semirares, as they appear
    try {
	handling_counters = true;
...
void drink_up()
{
    ...
    // Perhaps drink a Lucky Lindy to find the semirare counter
    if ( drink_lucky_lindy() ) {
    ...
    // We might want to reserve drunkenness for future Lucky Lindys
    int reserve = will_hunt_semirares ? max( 0, 3 - get_property( "_speakeasyDrinksDrunk" ).to_int() ) : 0;
...
void spleen_up()
{
    ...
    // If we are not going to do any of those things, leave spleen alone
    if ( !will_synthesize && !will_dmt && !will_calculate ) {
	return;
    }
...
void lubricate_tracks()
{
	...
	do_adventure_until_choice( my_adventures(), BARF_MOUNTAIN, 1073 );
...
void pantsgiving_adventures()
{
    // Pantsgiving could have given us some fullness
    int current_full = my_fullness();
    int max_full = fullness_limit();

    while ( current_full < max_full ) {
	int full_remaining = max_full - current_full;
	...
	item food = item_amount( HORSERADISH ) >= full_remaining ? HORSERADISH : PIZZA;
	eat_food( full_remaining, food );

	// Eating with Mayo could leve us improperly equipped, if we
	// used Travoltan trousers to buy it
	suit_up( farm_location );

	// Allow for 1 choice adventure before a combat increases fullness
	do_adventure( 2, farm_location );

	current_full = my_fullness();
	max_full = fullness_limit();
    }

    do_adventure( my_adventures(), farm_location );
}
...
void comb_beach()
{
    if ( !have_beach_comb ||
	 ( beach_comb_turns < 0 ) ||
	 !svn_exists( "veracity0-beach" )) {
	return;
    }

    if ( beach_comb_turns == 0 ) {
	// Only want free walks. How many have we used?
	int free_walks_used = get_property( "_freeBeachWalksUsed" ).to_int();
	if ( free_walks_used < 11 ) {
	    cli_execute( "BeachComber free" );
	}
    } else {
	// We are willing to spend turns. How many turns have we spent today
	// using the BeachComber script?
	int turn_walks_used = get_property( "_VBC.TurnsSpentCombing" ).to_int();
	if ( beach_comb_turns > turn_walks_used ) {
	    // We have not yet used all we are willing to use
...
void bastille_battalion()
{
    ...
    if ( !svn_exists( "Ezandora-Bastille-branches-Release" )) {
	return;
    }
...
void detective_school()
{
    if ( !have_detective_school ||
	 ...
	 !svn_exists("Ezandora-Detective-Solver-branches-Release")) {
	return;
    }
...
void run_tasks()
{
	...
	// Consume Pantsgiving adventures
	pantsgiving_adventures();
